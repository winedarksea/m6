...
period = "4"
train_test = True
highlevel_validations = 3  # not 0 based
...
for trial in range(100):
    ...
    run = f"run_{trial}"
    current_run = run_list[0]
    current_run = {**current_run, **AutoTS.get_new_params(method='superfast')}
    current_run = AutoTS.get_new_params(method='fast')
    # current_run['max_generations'] = gens if current_run['max_generations'] > gens else current_run['max_generations']
    current_run['drop_most_recent'] = drop_most_recent
    try:
        for val in range(highlevel_validations):
            ...
            regr_train, regr_fcst = create_regressor(
                regr_df_weekly.reindex(df_train.index),
            ...
            if train_test:
                ...
                metric_df['params'] = json.dumps(current_run)
        ...
        print(repr(e))
        failed_params.append(current_run)
    ...
    overall_result.to_csv("a_m6_trial_results.csv")
